...
solace-refresh-config uwsm/default
solace-refresh-config uwsm/env
...
if grep -q "bindd = SUPER, N, Neovim" ~/.config/hypr/bindings.conf; then
  sed -i '/SUPER, N, Neovim, exec/ c\bindd = SUPER, N, Editor, exec, solace-launch-editor' ~/.config/hypr/bindings.conf
fi
