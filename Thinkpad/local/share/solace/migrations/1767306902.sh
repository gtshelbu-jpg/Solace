...
# Move user-added backgrounds from Solace theme folders to user config
SOLACE_DIR="$HOME/.local/share/solace"
USER_BACKGROUNDS_DIR="$HOME/.config/solace/backgrounds"

if [[ -d $SOLACE_DIR/themes ]]; then
  cd "$SOLACE_DIR"

  # Get list of git-tracked background files (relative to solace dir)
...
THEMES_DIR="$HOME/.config/solace/themes"
CURRENT_THEME_LINK="$HOME/.config/solace/current/theme"
...
elif [[ -f $HOME/.config/solace/current/theme.name ]]; then
  CURRENT_THEME_NAME=$(cat "$HOME/.config/solace/current/theme.name")
fi

# Remove all symlinks from ~/.config/solace/themes
...
if [[ -n $CURRENT_THEME_NAME ]]; then
  solace-theme-set "$CURRENT_THEME_NAME"
else
  # Backup to ensure a theme is set if we can't deduce the name
  solace-theme-set "Tokyo Night"
fi
