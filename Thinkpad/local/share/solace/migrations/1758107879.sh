...
for pkg in "${PACKAGES[@]}"; do
  if ! solace-pkg-present "$pkg"; then
...
if $NEEDS_MIGRATION; then
  ...
  solace-pkg-drop walker-bin walker-bin-debug

  solace-pkg-add "${PACKAGES[@]}"

  source $SOLACE_PATH/install/config/walker-elephant.sh

  rm -rf ~/.config/walker/themes

  solace-refresh-config walker/config.toml
  solace-refresh-config elephant/calc.toml
  solace-refresh-config elephant/desktopapplications.toml
fi
