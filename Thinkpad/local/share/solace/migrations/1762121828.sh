...
# https://github.com/basecamp/solace/issues/1852
...
if command -v alacritty > /dev/null 2>&1; then
  cp "$SOLACE_PATH/applications/Alacritty.desktop" ~/.local/share/applications/
fi
...
if [[ -f $waybar_config ]]; then
  sed -i 's|"on-click-right": "solace-launch-terminal"|"on-click-right": "xdg-terminal-exec"|' "$waybar_config"
  sed -i 's|"on-click": "\$TERMINAL -e btop"|"on-click": "xdg-terminal-exec btop"|' "$waybar_config"
  sed -i 's|"on-click": "\$TERMINAL --class=Wiremix -e wiremix"|"on-click": "xdg-terminal-exec --app-id=com.solace.Wiremix -e wiremix"|' "$waybar_config"
  solace-state set restart-waybar-required
fi

# Update hyprland window rules to use DNS-format class names
system_conf=~/.config/hypr/apps/system.conf
if [[ -f $system_conf ]]; then
  if grep -q 'class:(.*|Impala|' "$system_conf" || grep -q 'class:(.*|Wiremix|' "$system_conf" || grep -q '|Solace|' "$system_conf"; then
    sed -i 's/\bImpala\b/com.solace.Impala/g; s/\bWiremix\b/com.solace.Wiremix/g; s/|Solace|/|com.solace.Solace|/g' "$system_conf"
  fi
fi
