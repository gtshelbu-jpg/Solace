...
# Users can still opt-in to Cloudflare DNS using: solace-setup-dns cloudflare
...
if [[ -f /etc/systemd/resolved.conf ]]; then
  ...
  if [[ -f /etc/systemd/network/99-solace-dns.network ]]; then
    sudo rm -f /etc/systemd/network/99-solace-dns.network
  ...
  echo "To use Cloudflare DNS, run: solace-setup-dns Cloudflare"
fi