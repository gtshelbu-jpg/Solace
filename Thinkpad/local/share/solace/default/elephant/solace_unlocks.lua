--
-- Dynamic Solace Unlocks Menu for Elephant/Walker
--
-- A "Default" entry restores the solace-shipped Plymouth via
-- solace-plymouth-reset. After that, every theme that has a preview-unlock.png
-- appears as a customised unlock; picking one runs solace-plymouth-set-by-theme
-- <theme>. Both run in a floating terminal so sudo can prompt.
--
Name = "solaceunlocks"
NamePretty = "Solace Unlocks"
...
function GetEntries()
  ...
  local user_themes_dir = home .. "/.config/solace/themes"
  local solace_path = os.getenv("SOLACE_PATH") or ""
  local default_themes_dir = solace_path .. "/themes"
  local default_preview = solace_path .. "/default/plymouth/preview-unlock.png"
      ...
      if theme_name and not seen_themes[theme_name] then
        ...
        if file_exists(preview_path) then
          ...
          table.insert(entries, {
            ...
            PreviewType = "file",
            Actions = {
              activate = "solace-launch-floating-terminal-with-presentation "
                .. shell_escape("solace-plymouth-set-by-theme " .. shell_escape(theme_name)),
            },
          })
        end
      end
  ...
  local default_entry = {
    ...
    Actions = {
      activate = "solace-launch-floating-terminal-with-presentation "
        .. shell_escape("solace-plymouth-reset"),
    },
  }
