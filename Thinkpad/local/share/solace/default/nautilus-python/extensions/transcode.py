...
class TranscodeAction(GObject.GObject, Nautilus.MenuProvider):
    def _launch_transcode(self, paths):
        wrapper = shutil.which("solace-launch-floating-terminal-with-presentation")
        binary = shutil.which("solace-transcode")
    ...
    def _make_item(self, paths):
        ...
        item = Nautilus.MenuItem(
            name="SolaceTranscodeNautilus::transcode",
            label=label,
    ...
    def _tools_available(self):
        return bool(
            shutil.which("solace-launch-floating-terminal-with-presentation")
            and shutil.which("solace-transcode")
        )
